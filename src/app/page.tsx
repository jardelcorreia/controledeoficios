...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setError("Falha ao conectar com o banco de dados. Verifique os índices do Firestore.");
      setLoading(false);
    });
  ...
  useEffect(() => {
    ...
    const unsubscribeConfig = onSnapshot(configRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        // Nota: Esta consulta exige um índice composto no Firestore: ano (ASC) + numeroSequencial (DESC)
        const qUltimo = query(
          collection(db, "oficios"),
        ...
        const unsubscribeUltimo = onSnapshot(qUltimo, (ultimoSnap) => {
          ...
          console.error("Erro ao calcular próximo número (provável falta de índice):", err);
          // Se falhar a consulta complexa, tentamos uma simplificada ou exibimos erro
          setError("Aviso: O cálculo automático do número pode exigir a criação de um índice no console do Firebase.");
        });
      ...
      <main className="flex-1 p-4 sm:p-6 space-y-6">
            ...
            <CardContent className="flex-1 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Este será o número do próximo ofício a ser criado automaticamente.
                </p>
              </div>
              <NovoOficioDialog
        ...
        <Card className="shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Últimos documentos gerados. Atualizado em tempo real.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
